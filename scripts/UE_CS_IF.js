...
function afterSubmit(type){
	...
	if(type == "edit" || type == "create") {
		...
		nlapiLogExecution('Debug', 'AfterSubmit Is Started : ', "Item Fulfillment Is Created.");
		...
		if (createdFrom != '' || createdFrom != null) {
			try {
				...
				var pcif={
					...
					"trac_num"			:nlapiGetLineItemValue('package','packagetrackingnumber', 1),
					"cs_invoice_id"		:createInvoice(createdFrom),
					"cs_if_id"			:nlapiGetRecordId()
				};
				nlapiLogExecution('Debug', "JSON_TEXT_PAYLOAD" , JSON.stringify(pcif, replacer));

				ifresp = sendIFtoPC(pcif);

				if(ifresp.success) {
					
					nlapiSubmitField('itemfulfillment', nlapiGetRecordId(), 'custbody_pc_if_id', ifresp.ifid);
					nlapiSubmitField('itemfulfillment', nlapiGetRecordId(), 'custbody_pc_invid', ifresp.vbId);
				}
...
//		var restletUrl = 'https://rest.na1.netsuite.com/app/site/hosting/restlet.nl?script=321&deploy=1';
		var restletUrl = 'https://rest.sandbox.netsuite.com/app/site/hosting/restlet.nl?script=290&deploy=1';
		var token = {
			key: '5feaa2c312120873fbf35af7955c92da15c394ac59fedd013365158805c8ed82',
			secret: '11f014700f63a2be67921ef3fed2b3d569b08eb0b2e9ac960c9601bff6f15e79'
		};

		var oauth = OAuth({
			consumer: {
				key: '4fa7a632f12853c57bd25c9cb99f977d8d6747dcd04dce005ecdf0d84c337cd0',
				secret: 'fd079de9c7d4058b3ae1a2011c7fe7bc08fb90a4c827082f28e5aa608e45d609'
			},
		...
		nlapiLogExecution('Debug', "headerWithRealm.Authorization" , headerWithRealm.Authorization);
		...
		var restResponse = nlapiRequestURL(restletUrl, JSON_TEXT_PAYLOAD, headers, HTTP_METHOD);
...
function createInvoice(createdFrom){
	try{
		...
		for (var i = 1; i <= nlapiGetLineItemCount('item'); i ++){
			...
			record.setCurrentLineItemValue('item', 'item', nlapiGetLineItemValue('item', 'item', i));
			record.setCurrentLineItemValue('item', 'quantity', nlapiGetLineItemValue('item', 'quantity', i));
			record.setCurrentLineItemValue('item', 'amount', soRec.getLineItemValue('item', 'amount', i));
