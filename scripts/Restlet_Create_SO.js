...
function createSO(JSON_Data) {
	try
	{
		...
		if(entityID != 0) {
			...
			createErrorLogRecord(JSON_Data, "Customer Not Found - " + JSON_Data.email);
			return null;
		}
		...
		so.setFieldValue( 'shipaddr1',			shippingAddress['shipaddr1'] + ' ' + shippingAddress['shipaddr2']);
...
//		so.setFieldValue( 'custbody_pc_so_id', JSON_Data.pc_so_id);
		...
		if(shippingMethod != 0){
			...
			createErrorLogRecord(JSON_Data, "ShipMethod Not Found - " + JSON_Data.shipmethod);
			return null;
		}
		...
		for( z = 0; z < JSON_Data.products.length; z++ ) {
			...
			if(internalId != 0) {
				...
				so.setLineItemValue('item', 'quantity', z+1, JSON_Data.products[z]['quantity']);
				so.setLineItemValue('item', 'amount', z+1, JSON_Data.products[z]['amt']);
//				so.setLineItemValue('item', 'price', z+1, JSON_Data.products[z]['unitprice']);
				so.setLineItemValue('item', 'rate', z+1, JSON_Data.products[z]['unitprice']);
			} else {
				createErrorLogRecord(JSON_Data, "Product Not Found, UPCCode - " + JSON_Data.products[z]['upccode']);
				return null;
			}
		}
		...
		createErrorLogRecord(JSON_Data, 'Sales Order is created with success. Created SO ID : ' + newso, "T");
...
//	var simple_data = removeFromObjectByKey(data, 'logo');
	delete data.logo;
	delete data.sopdf;
...
function createPDFFile(file_content, soid){
	try {
		...
		//filex.setEncoding(file_content.encd);
		filex.setFolder(612);
		var fid = nlapiSubmitFile(filex);
		nlapiAttachRecord('file', fid, 'salesorder', soid);
		return fid;
